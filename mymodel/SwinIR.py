...
class SwinEncoder(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
        ...
        # 输出
        self.lrelu=nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.fc=nn.Linear(img_size*img_size, 1)
    ...
    def forward(self, x):
        ...
        x = self.fc(self.lrelu(x).view(x.shape[0], self.embed_dim, -1)).squeeze(-1)
...
if __name__ == '__main__':
    ...
    encoder=SwinEncoder(img_size=16, patch_size=1, in_chans=1,
        embed_dim=256, depths=[6, 6, 6, 6], num_heads=[4, 4, 4, 4],
        window_size=1, mlp_ratio=4., qkv_bias=True, qk_scale=None,
    )

    test_input=torch.randn(8,1,16,16)
