...
import utils
import record
from EBCF_CDEM import dataset
from hyper_demo.model import hyper_net,process_batch
...
if __name__ == '__main__':
    ...
    recorder=record.Recorder(logger=logging.getLogger(__name__),
                             output_path=r'D:\codes\cdem\output\hyper_demo',
    ...
    # -----------------------------------------#
    # 模型，优化器，调度器
    model = hyper_net(1,image_size=16).to(device)
    model.to(device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        with tqdm(total=len(train_loader), desc=f'epoch {epoch}/{epochs} train', file=sys.stdout) as t:
            for input, hr_coord, gt, trans in train_loader:
                ...
                params= model(input)
                sr_value=process_batch(params, input, hr_coord)
